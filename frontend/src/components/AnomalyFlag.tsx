...
export function AnomalyFlag({ flagType, description, compact = false }: Props) {
  ...
  if (compact) {
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-label-sm font-medium ${c.bg} ${c.text} ${c.border}`}>
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg border ${c.bg} ${c.border}`}>
      ...
      <div>
        <p className={`text-body-md font-semibold ${c.text}`}>{c.label}</p>
